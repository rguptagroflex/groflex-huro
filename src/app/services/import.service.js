...
class ImportService {
            ...
            if (
              ...
              if (
                ...
                if (entry.address[column.key].length === 2) {
                  ...
                  entry.address[column.key] = selectedCountry
                ...
                column.key === "countryIso" &&
                (entry.address[column.key] === "" ||
                  entry.address[column.key] === null ||
                  entry.address[column.key].length <= 1)
              ) {
                entry.address[column.key] = "IN";
              } else if (["street", "city"].indexOf(column.key) >= 0) {
                entry.address[column.key] = row[index]
                  ? toProperCase(row[index])
                  : "";
              }
            } else if (
              ["stateName", "category", "firstName", "lastName"].indexOf(
                column.key
              ) >= 0
            ) {
              ...
              entry[column.key] =
                row[index] && typeof row[index] !== "number"
                  ? row[index].replace(/\s+/g, "")
                  : "";
              if (["email", "type", "kind"].indexOf(column.key) >= 0) {
                entry[column.key] = entry[column.key]
                  ? entry[column.key].toLowerCase()
                  : "";
              }
            } else {
              entry[column.key] =
  ...
  mapErrors(errors, type) {
    const fields =
      type === "customers" ? getCustomerFields() : getArticleFields();
    ...
    errors.forEach((error) => {
      ...
      if (field) {
        error.description = `${getResource("str_row")} ${error.index + 1}, ${
          field.label
        } ${reason ? reason.label : getResource("str_invalid")}`;
      }
    });
      ...
      data.forEach((field, index) => {
        ...
        fields.forEach((column) => {
          if (Array.isArray(column.required)) {
            ...
            if (
              !field[key] ||
              (field[key].toLowerCase() === value.toLowerCase() &&
                !field[column.key])
            ) {
              invalidColumns.push(column);
            }
          } else if (column.allowed) {
            column.allowed = column.allowed.map(function (x) {
              return x.toLowerCase();
            });
            if (column.required) {
              if (
                field[column.key] &&
                column.allowed.indexOf(field[column.key].toLowerCase()) === -1
              ) {
                invalidColumns.push(column);
              } else if (
                field[column.key] === null ||
                field[column.key] === ""
              ) {
                invalidColumns.push(column);
              ...
              if (
                ...
                column.allowed.indexOf(
                  field["address"][column.key].toLowerCase()
                ) === -1
              ) {
                invalidColumns.push(column);
              }
            ...
            column.key === "discount" &&
            (field[column.key] < 0 || field[column.key] > 100)
          ) {
            invalidColumns.push(column);
          }
        });
  ...
  parseInput(input) {
    ...
    if (input.length > 0) {
      ...
      input.forEach((columns) => {
        ...
        if (rowHasValues) {
          columnCount =
            columns.length > columnCount ? columns.length : columnCount;
          rows.push(row);
        }
      });
    }
