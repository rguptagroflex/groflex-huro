...
const removePrepaddedZeros = (timeStringSlice) => {
  return timeStringSlice.match(/^0+$/g, "")
    ? "0"
    : timeStringSlice.replace(/^0*/g, "");
};
...
const sanitizeTimeString = (timeString) => {
  ...
  if (
    ...
    const minutes =
      sanitizedTimeSlices[1].length === 2
        ? sanitizedTimeSlices[1]
        : `0${sanitizedTimeSlices[1]}`;
...
export const isValid24HourTimeObject = (timeObject) => {
  ...
  const isWithinMinuteBoundary =
    timeObject.minute >= 0 && timeObject.minute < 60;
  return isWithinHourBoundary && isWithinMinuteBoundary;
};

export const calculateDiffBetweenTwoTimeStrings = (
  startTimeString,
  endTimeString
) => {
  ...
  if (
    ...
    (timeStartObject.hour === timeEndObject.hour &&
      timeStartObject.minute < timeEndObject.minute)
  ) {
    if (timeStartObject.minute <= timeEndObject.minute) {
      calculatedTrackedTime.minute =
        timeEndObject.minute - timeStartObject.minute;
    } else {
      calculatedTrackedTime.minute =
        60 - (timeStartObject.minute - timeEndObject.minute);
      calculatedTrackedTime.hour = -1;
    }
    calculatedTrackedTime.hour =
      calculatedTrackedTime.hour + (timeEndObject.hour - timeStartObject.hour);
  }
