...
export default class Timetracking {
  constructor(data) {
    ...
    this.startDate =
      data.startDate ||
      moment().set(ZERO_THOROUGH_TIMEOBJECT).format(config.datetimeFormat.api);
    this.endDate =
      data.endDate ||
      moment().set(ZERO_THOROUGH_TIMEOBJECT).format(config.datetimeFormat.api);
  ...
  get date() {
    const date =
      this.startDate === config.dateEpochValue
        ? moment()
    ...
    if (this.durationInMinutes > 0) {
      ...
      if (
        ...
        calculatedTrackedTime.hour =
          calculatedTrackedTime.hour +
          (timeEndObject.hour - timeStartObject.hour);
      }
  ...
  get displayEffort() {
    if (this.status !== "invoiced") {
      return this.rowCount > 1
        ? `${this.rowCount} ${getResource("str_expensesOpen")}`
        : `${this.rowCount} ${getResource("str_effortOpen")}`;
    } else {
      return this.rowCount > 1
        ? `${this.rowCount} ${getResource("str_expenseSettled")}`
        : `${this.rowCount} ${getResource("str_expenseBilled")}`;
    }
  }
}
