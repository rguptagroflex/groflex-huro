...
export default class DeliveryChallan {
  constructor(data) {
    data = !data ? {} : data;
    console.log(data);
    ...
    this.acceptUserAgreement =
      !!data.acceptUserAgreement || !!data.useAdvancedPaymentOptions;
    ...
    const deliveryPeriodField =
      data.infoSectionFields &&
      data.infoSectionFields.find(
        (f) => f.name === "deliveryPeriod" && f.active
      );
    this.deliveryPeriod =
      (deliveryPeriodField && deliveryPeriodField.value) || "";
    ...
    this.state = data.state || DeliveryChallanState.OPEN;
    ...
    this.additionalCharges =
      ...
      Object.assign({}, { shippingCharge: 0, serviceCharge: 0 });
  ...
  get dueDateKind() {
    ...
    const userDate = moment(this.dueToDate, [
  ...
  get isCancelled() {
    return this.state === DeliveryChallanState.CANCELLED;
  }
  get isDeclined() {
    return this.state === DeliveryChallanState.DECLINED;
  }
  ...
  get isInvoiced() {
    return this.state !== DeliveryChallanState.INVOICED;
  }

  get isDelivered() {
    return this.state !== DeliveryChallanState.DELIVERED;
  }
  ...
  setCustomer(customer) {
    ...
    if (
      (contact || (contactPersons && contactPersons.length > 0)) &&
      kind === "company"
    ) {
